...
public class LakituMove : MonoBehaviour
{
    ...
    public float marioRangeX;
    public float marioRangeY;
    private float moveCounter;
    private bool moveRight;
    private Vector3 nextPos;
    private float parabola;
    private float distance;
    public int movementsBeforeSpiny;
    ...
    void Start()
    {
        ...
        leftPosition = new Vector2(marioMove.transform.parent.transform.position.x-marioRangeX,marioMove.transform.parent.transform.position.y+marioRangeY);
        transform.parent.transform.position = leftPosition;
        rightPosition = new Vector2(marioMove.transform.parent.transform.position.x+marioRangeX, marioMove.transform.parent.transform.position.y+marioRangeY);
        moveCounter = 0;
        moveRight = true;
        pauseTimer = 0;
    ...
    void FixedUpdate()
    {
        ...
        if (pauseTimer <= 0)
        {
            if (isFallingAttacking && !isSpinnyAttacking)
            {
                ...
                if (progress == 1)
                {
                    if (marioMove.transform.parent.transform.position.x < rb.transform.position.x) moveRight = false;
                    else moveRight = true;
                    isFallingAttacking = false;
                }
            }
            else
            {
                ...
                if (transform.position.Equals(leftPosition) || transform.position.Equals(rightPosition))
                {
                    ...
                    if (moveCounter >= movementsBeforeSpiny && !isSpinnyAttacking)
                    {
                        ...
                        progress = 0;
                        arcHeight = transform.position.y - marioMove.transform.position.y;
                    }
                    moveRight = !moveRight;
