...
public class MagicianMovesController : MonoBehaviour
{
    ...
    public float attackRechargeTimerValue;
    ...
    void FixedUpdate()
    {
        ...
        distance = (float)Math.Sqrt(Math.Pow(marioMove.transform.parent.transform.position.x - transform.parent.transform.position.x, 2) + Math.Pow(marioMove.transform.parent.transform.position.y - transform.parent.transform.position.y, 2));
        ...
        if (!isUsingShield && !isSpawningGoombasAttacking && !isStunAttacking && distance < distanceValue) {
            ...
            if (marioMove.transform.parent.transform.position.x < rb.transform.position.x) facingeRight = false;
